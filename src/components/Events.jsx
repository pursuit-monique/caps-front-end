...
export default function Events() {
  ...
  const eventCardType =
    userAgent === "mobile"
      ? "flex-column heightmenu overflow-auto order-2"
      : "flex-column heightmenu overflow-auto order-1";
    ...
    <>
      ...
      <article className="d-flex flex-wrap body">
        ...
        <div className={eventCardType}>
          <EventCard
            currEvents={currEvents.filter((event) =>
              !!category.id ? event.cause_id === Number(category.id) : true
            )}
            mapCenter={mapCenter}
            markerId={markerId}
            userAgent={userAgent}
